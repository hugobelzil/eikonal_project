...
\noindent In this report, we will focus on solving Eikonal equations in 2 space dimensions. We will introduce the discretization and fast-sweeping method proposed by H.Zhao in 2008 \cite{zhao2005}, considering several scenarios for $f(x,y)$ and $\Gamma$, and observing some numerical results. Finally, we will use these results and apply them to an optimal control problem : path planning.
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      title={Bilinear interpolation in a mesh cell},
      xlabel={$x$}, ylabel={$y$}, zlabel={$u$},
      grid=major, view={50}{25}
    ]
      % --- corner values -----------------------------------------------------
      \addplot3[color=blue,mark=*] coordinates
        {(0,0,0.3) (1,0,0.7) (1,1,1) (0,1,0.5)};

      % --- cell edges --------------------------------------------------------
      \addplot3[color=red,line width=0.5pt] coordinates {(0,0,0.3) (1,0,0.7)};
      \addplot3[color=red,line width=0.5pt] coordinates {(1,0,0.7) (1,1,1)};
      \addplot3[color=red,line width=0.5pt] coordinates {(1,1,1) (0,1,0.5)};
      \addplot3[color=red,line width=0.5pt] coordinates {(0,1,0.5) (0,0,0.3)};

      % --- centre point in (x,y)-plane --------------------------------------
      \addplot3[color=black,mark=*] coordinates {(0.5,0.5,0)};

      % dotted vertical to interpolated height
      \addplot3[color=black,dotted] coordinates
        {(0.5,0.5,0) (0.5,0.5,0.625)};

      % interpolated value on the surface
      \addplot3[color=black,mark=*,mark options={fill=white}] coordinates
        {(0.5,0.5,0.625)};

      % ----------------------------------------------------------------------
      % NEW: dotted line on the bilinear surface, x = 0.5 (up–down direction)
      % Plane equation through the two side edges:
      %   z(x,y) = 0.3 + 0.4 x + 0.25 y
      % At x = 0.5 → z = 0.3 + 0.4(0.5) + 0.25 y = 0.5 + 0.25 y
      \addplot3[color=black,dotted] coordinates
        {(0.5,0,0.5) (0.5,1,0.75)};
      % ----------------------------------------------------------------------
    \end{axis}
  \end{tikzpicture}
  \caption{Bilinear interpolation inside one grid cell:
           corner data (blue), cell edges (red), centre point,
           vertical projection, interpolated value (white-filled mark),
           and the dotted surface line in \(y\)-direction.}
  \label{fig:bilinear_cell}
\end{figure}
...
\noindent It is particularly interesting to see that the path indeed avoids these regions with low velocities, which can be visualized more easily on figure (\ref{3d_surface_mountain}). We introduced a "fast lane" in the second example, where we set $F(x,y)=6$, and this completely modifies the trajectory of the optimal path in time.
