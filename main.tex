...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.5\textwidth]{plots/bilinear-interpolation.jpg}
...
\label{interpolation}
\begin{aligned}
\tilde{u}(x,y) &= \beta u_b + (1-\beta)u_r \\[4pt]
               &= \beta\bigl( \alpha u_{11} + (1-\alpha)u_{12} \bigr)
                  + (1-\beta)\bigl( \alpha u_{21} + (1-\alpha)u_{22} \bigr)
\end{aligned}
\end{equation}
...
\begin{equation}
    \begin{cases}
        \frac{x_1^{n+1}-x_1^n}{\Delta t}=-\tilde{u_x}(x_1^n,x_2^n) \\
        \frac{x_2^{n+1}-x_2^n}{\Delta t}=-\tilde{u_y}(x_1^n,x_2^n)
    \end{cases} \implies
    \begin{cases}
        x_1^{n+1} = x_1^n-\Delta t\tilde{u_x}(x_1^n,x_2^n) \\
        x_2^{n+1} = x_2^n-\Delta t\tilde{u_y}(x_1^n,x_2^n)
    \end{cases}
\end{equation}

\noindent which we recognize as being a gradient descent, stopping when $[x_1^n,x_2^n]^T\in \Gamma$, and where $\tilde{u_x}$ and $\tilde{u_y}$ are the interpolated first derivatives of the numerical solution $u$.
...
\begin{figure}[h!]
  ...
  _t$}
  \label{fig:loglog_conv_hausdorff_large}
\end{figure} 
